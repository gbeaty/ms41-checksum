...
unsigned short crc16_ccitt(const void *buf, int len, unsigned short initial)
{
...
void check(unsigned char* contents, unsigned int size, struct Checks cs) {
	...
	for(i=0; i<cs.num; i++) {
		...
		(*c).recorded = getWord(contents, checksumAddr);
		...
		(*c).calculated = crc16_ccitt(contents + addr, (*c).size, initialValue);
...
struct Checks checkFile(FILE* fp) {
	...
	cs.start = malloc(sizeof(struct Check) * cs.num);
