...
#ifdef BMP_POF2
#  define OFS(var) ((BMP_BIT - 1UL) & ~(var))
#else
#  define OFS(var) (BMP_BIT - 1UL - (var) % BMP_BIT)
#endif
...
pshift(poly_t *dest, const poly_t src, unsigned long head, unsigned long start, unsigned long end, unsigned long tail) {
	...
	if(size)
		dest->bitmap[datidx] &= ~BMP_C(0) >> head % BMP_BIT;

	/* clear tail */
	if(length % BMP_BIT)
		dest->bitmap[size - 1UL] &= ~(~BMP_C(0) >> length % BMP_BIT);
...
prev(poly_t *poly) {
	...
	unsigned long ofs = (BMP_BIT - poly->length % BMP_BIT) % BMP_BIT;
...
prevch(poly_t *poly, int bperhx) {
	...
	for(iter = bperhx - 1; iter < poly->length; iter += bperhx) {
...
prcp(poly_t *poly) {
	...
	praloc(poly, (BMP_BIT - poly->length % BMP_BIT) % BMP_BIT + poly->length);
	prev(poly);
	first = pfirst(*poly);
	if(first >= poly->length) {
		pfree(poly);
...
pinv(poly_t *poly) {
	...
	if(poly->length % BMP_BIT)
		poly->bitmap[size - 1UL] &= ~(~BMP_C(0) >> poly->length % BMP_BIT);
}
...
pcrc(const poly_t message, const poly_t divisor, const poly_t init, const poly_t xorout, int flags) {
	...
	if(divisor.length <= (unsigned long) BMP_BIT
		 ...
		 */
		palloc(&result, (init.length > divisor.length ? init.length : divisor.length) + (BMP_BIT << 1));
		...
		for(iter = 0UL, ofs = 0UL; iter < max; ++iter, probe >>= 1) {
			if(!probe) {
				 ...
				 */
				*rptr++ = *sptr++ ^ *bptr++;
				for(resiter = BMP_BIT << 1; resiter < result.length; resiter += BMP_BIT)
					*rptr++ = *sptr++;
			}
			++ofs;
...
praloc(poly_t *poly, unsigned long length) {
	...
	if(poly->bitmap) {
		if(poly->length < length) {
			 ...
			 */
			if(poly->length % BMP_BIT)
				poly->bitmap[oldsize - 1UL] &= ~(~BMP_C(0) >> poly->length % BMP_BIT);
			while(oldsize < size)
				poly->bitmap[oldsize++] = BMP_C(0);
		} else if(length % BMP_BIT)
			/* poly->length >= length > 0.
			 * poly shrunk. clear new last word
			 */
			poly->bitmap[size - 1UL] &= ~(~BMP_C(0) >> length % BMP_BIT);
		poly->length = length;
	} else
...
rev(bmp_t accu, int bits) {
	...
	result = result << bits | revtab[accu & 0xff] >> (8 - bits);
	return(result);
}
